...
public class AdminFacade extends DefaultUser {
    ...
    public static int createTable(String drawerName, String guesserName) throws DbBusinessException {
        try {
            DBManager.startTransaction();
            PlayerDto drawer = getPlayer(drawerName);
            PlayerDto guesser = getPlayer(guesserName);

            int i = GameBr.add(new GameDto(drawer.getId(), guesser.getId(), new Date()));
            DBManager.validateTransaction();
            return i;
        } catch (DbException eDB) {
            String msg = eDB.getMessage();
    ...
    public static void leaveTable(String playerName, int id) throws DbBusinessException {
        try {
            ...
            if (game.getEndTime() == null) {
                GameBr.update(new GameDto(id, game.getDrawer(), game.getPartner(), game.getStartTime(), new Date(), player.getId(), 0));
            }
            DBManager.validateTransaction();
        } catch (DbException eDB) {
            String msg = eDB.getMessage();
    ...
    public static void gameWon(int id) throws DbBusinessException {
        try {
            ...
            GameDto game = getGameById(id);
            if (game.getEndTime() == null) {
                GameBr.update(new GameDto(id, game.getDrawer(), game.getPartner(), game.getStartTime(), new Date(), 0, 0));
            }
            DBManager.validateTransaction();
        } catch (DbException eDB) {
            String msg = eDB.getMessage();
